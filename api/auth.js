 ...
 * v4.1.0 - 双重配额版本
 * 
 * 新增：搜索配额（searchQuota/searchCount）
 */
...
async function handleRegister(body, res) {
  ...
  const userData = {
    ...
    exportCount: '0',
    personalQuota: '0',
    searchCount: '0',      // 【新增】已使用搜索次数
    searchQuota: '0',      // 【新增】搜索配额
    maxDevices: '1',
    currentToken: '',
    tokenCreatedAt: '0',
...
async function handleLogin(body, res) {
  ...
  return res.status(200).json({
    ...
    user: {
      ...
      searchCount: parseInt(user.searchCount) || 0,      // 【新增】
      searchQuota: parseInt(user.searchQuota) || 0       // 【新增】
    },
    announcement
  });
}
...
async function handleVerify(body, res) {
  ...
  // 【新增】搜索配额信息
  let searchQuotaInfo = null;
  if (searchQuota > 0) {
    searchQuotaInfo = {
      quota: searchQuota,
      used: searchCount,
      remaining: searchQuota - searchCount
    };
  }
