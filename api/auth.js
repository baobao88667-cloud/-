...
async function handleRegister(body, res) {
  ...
  const userData = {
    ...
    personalQuota: '0',      // 个人积分总量（0表示无限制或使用团队积分）
    maxDevices: '1',
    currentToken: '',
    tokenCreatedAt: '0',
...
async function handleLogin(body, res) {
  ...
  if (user.status === 'locked') {
    return res.status(403).json({ ok: false, msg: '账号已被锁定（积分用完）', code: 'LOCKED' });
  }
  ...
  return res.status(200).json({
    ...
    user: {
      ...
      line: user.line || '',
      expireAt: expireAt || 0,
      usedCount: parseInt(user.exportCount) || 0,      // 已使用积分
      personalCredits: parseInt(user.personalQuota) || 0  // 个人积分总量
    },
    announcement
  });
}
...
async function handleVerify(body, res) {
  ...
  if (personalCredits > 0) {
    ...
    creditsInfo = {
      ...
      used: usedCount,
      remaining: personalCredits - usedCount
    };
  } else if (user.line) {
    // 团队积分
    ...
    if (lineData) {
      ...
      if (lineCredits > 0) {
        creditsInfo = {
          ...
          credits: lineCredits,
          used: lineUsed,
          remaining: lineCredits - lineUsed
        };
      }
    }
  }
