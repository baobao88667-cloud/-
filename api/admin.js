...
async function getVersionControl(res) {
  ...
  return res.status(200).json({
    ok: true,
    data: {
      enabled: versionControl.enabled === 'true',
      minVersion: versionControl.minVersion || '0.0.0',
      lockMessage: versionControl.lockMessage || '已更新请找管理员拿最新版本'
    }
  });
}
...
/**
 * 设置版本控制
 * @param {Object} body - { enabled: boolean, minVersion: string, lockMessage: string }
 */
async function setVersionControl(body, res) {
  ...
  if (enabled && minVersion) {
    ...
    if (!versionRegex.test(minVersion)) {
      return res.status(400).json({ 
        ok: false, 
        msg: '版本号格式无效，请使用 x.y.z 格式（如 6.4.3）' 
      });
    }
  }
  
  await kv.hset('tps:version_control', {
  ...
  return res.status(200).json({ 
    ok: true, 
    msg: enabled ? `版本控制已启用，最低版本要求: ${minVersion}` : '版本控制已禁用'
  });
}
