  ...
  if (userMode === 'normal') {
    ...
    if (expireAt === 0) {
      return res.status(400).json({ 
        ok: false, 
        msg: '无法设置为正常用户：该用户没有设置有效期' 
      });
    }
    if (expireAt > 0 && Date.now() > expireAt) {
      return res.status(400).json({ 
        ok: false, 
        msg: '无法设置为正常用户：该用户的有效期已过期' 
      });
    }
    ...
    if (personalQuota > 0) {
      ...
      if (lineData) {
        ...
        if (lineQuota > 0 && lineUsed >= lineQuota) {
          return res.status(400).json({ 
            ok: false, 
            msg: '无法设置为正常用户：该用户的团队积分已用完' 
          });
        }
      } else {
        return res.status(400).json({ 
          ok: false, 
          msg: '无法设置为正常用户：该用户没有积分（无个人积分且团队不存在）' 
        });
      }
    } else {
      return res.status(400).json({ 
        ok: false, 
        msg: '无法设置为正常用户：该用户没有积分（无个人积分且未加入团队）' 
      });
    }
  }
...
async function batchSetUserMode(body, res) {
  ...
  for (const username of usernames) {
    ...
    if (userMode === 'normal') {
      ...
      // 检查有效期
      if (expireAt === 0) {
        results.failed.push({ username, reason: '没有有效期' });
        continue;
      }
      if (expireAt > 0 && Date.now() > expireAt) {
        results.failed.push({ username, reason: '有效期已过期' });
        continue;
      }
      ...
      if (!hasCredits) {
        results.failed.push({ username, reason: '没有可用积分' });
        continue;
      }
    }
