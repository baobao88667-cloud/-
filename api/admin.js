...
function calculateUserMode(user, lineData = null) {
  ...
  const timeExpired = expireAt > 0 && Date.now() > expireAt;
  ...
  if (personalCredits > 0) {
    hasCredits = usedCount < personalCredits;
    creditsExhausted = usedCount >= personalCredits;
  } else if (user.line && lineData) {
    const lineCredits = parseInt(lineData.quota) || 0;
    const lineUsed = parseInt(lineData.used) || 0;
    if (lineCredits > 0) {
      hasCredits = lineUsed < lineCredits;
      creditsExhausted = lineUsed >= lineCredits;
    } else {
      hasCredits = true;  // 团队积分为0表示无限制
    }
  }
  ...
  if (timeExpired && creditsExhausted) {
    return { mode: 'guest', reason: 'both_exhausted' };
  } else if (timeExpired) {
    ...
    return { mode: 'guest', reason: 'no_time_no_credits' };
  } else {
