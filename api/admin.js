...
async function getConfig(res) {
  ...
  // 【调试】打印原始配置值
  console.log('[getConfig] Raw config from Redis:', config);
  
  // 【修复】兼容多种可能的值格式
  const announcementEnabled = config.announcementEnabled === 'true' || config.announcementEnabled === true || config.announcementEnabled === '1';
  
  console.log('[getConfig] Parsed announcementEnabled:', announcementEnabled);
...
async function approveUser(body, res) {
  ...
  // 【修复】更新用户信息 - 审核通过同时启用用户，确保 enabled 为字符串 'true'
  ...
  // 【新增】验证写入是否成功
  const updatedUser = await kv.hgetall(`user:${username}`);
  console.log(`[approveUser] User ${username} updated:`, {
...
// 【修复】启用/禁用用户 - 增强错误处理和日志
async function toggleUser(body, res) {
  const { username, enabled } = body;

  console.log(`[toggleUser] Request received:`, { username, enabled, enabledType: typeof enabled });
  ...
  // 【修复】确保 enabled 值正确转换为字符串
  const enabledStr = (enabled === true || enabled === 'true' || enabled === 1 || enabled === '1') ? 'true' : 'false';
  
  console.log(`[toggleUser] Setting enabled to: ${enabledStr} for user: ${username}`);
  ...
  try {
    ...
    // 【新增】验证写入是否成功
    const updatedUser = await kv.hgetall(`user:${username}`);
    console.log(`[toggleUser] User ${username} after update:`, {
      enabled: updatedUser.enabled
    });

    const msg = enabledStr === 'true' ? '已启用' : '已禁用';
    return res.status(200).json({ ok: true, msg });
  } catch (error) {
    console.error(`[toggleUser] Error updating user ${username}:`, error);
    return res.status(500).json({ ok: false, msg: '更新失败: ' + error.message });
  }
}
...
async function batchSetQuota(body, res) {
  ...
  if (usernames && usernames.length > 0 && personalQuota !== undefined) {
    ...
    return res.status(200).json({ ok: true, msg: `已为 ${usernames.length} 个用户设置个人积分` });
  }
...
async function setConfig(body, res) {
  ...
  // 【调试】打印接收到的配置值
  console.log('[setConfig] Received body:', body);
  ...
  console.log('[setConfig] Saving config:', config);

  await kv.hset('system:config', config);

  // 【调试】验证保存后的值
  const savedConfig = await kv.hgetall('system:config');
  console.log('[setConfig] Config after save:', savedConfig);
